...
\]
where $\mu_{ijt}(\theta)$ is the predicted hourly trip rate from station $i$ to $j$ at time $t$, and $\lambda=10^{-3}$. The objective is convex and smooth, so we fit by L-BFGS-B \citep{Liu1989}.
