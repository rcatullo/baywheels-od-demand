...
\cite{Kochenderfer2022} step from $O(I^2 T)$ to $O(I^2)+O(T)$, making 1{,}258
...
elevation gain is negligible despite San Francisco's hilly terrain,
likely due to Lyft siting docks on level ground.
\end{abstract}
...
with a reanalysis weather product (ERA5-Land \cite{Munoz2021}) rather
...
\cite{Kochenderfer2022}.  Reference categories are pinned by setting
...
$(i,j)$ pairs.  San Francisco's hilly topography made this a
potentially informative covariate: steep climbs between docks were
expected to suppress demand on uphill OD pairs.

\textbf{Weather.}
ERA5-Land \cite{Munoz2021} provides hourly 9-km gridded reanalysis at
...
\textbf{Elevation.}
San Francisco's hills made elevation gain a motivating covariate for
this study: we expected climbs to measurably suppress demand on uphill
OD pairs.  The negligible fitted coefficient
($\hat\gamma_{\rm elev}\approx -10^{-5}$) was therefore surprising.
In retrospect, two mechanisms likely explain the null result.
The primary explanation is a structural one: Lyft appears to locate
docks on level, accessible ground, systematically avoiding steep
inclines for accessibility and operational reasons.  If dock-to-dock
elevation differences are small by design, the covariate lacks the
variance needed to produce a detectable coefficient regardless of the
true behavioural effect---a selection bias in the data-generating
process.  A secondary factor is signal-to-noise: even where elevation
differences exist, their effect on low-count cells (most OD pairs
average $\approx$1 trip/hr) may be too small to distinguish from
Poisson noise.  Both effects likely compound, and a model fit to a
system with deliberately hilly station siting would be a cleaner test
...
that area-wide weather adds genuine predictive signal.
