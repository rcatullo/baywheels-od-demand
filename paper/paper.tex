...
Figure~\ref{fig:decay} shows the fitted distance-decay curve for both
models: $\exp(\hat\gamma_{\rm dist}\cdot d)$ versus haversine distance
$d$.  The null and full model estimates are nearly identical
($\Delta\hat\gamma_{\rm dist}<0.001$), indicating that distance
dominates the spatial kernel and is not confounded by the weather terms.
...
\begin{figure}[t]
  ...
  \caption{Fitted distance-decay factor $\exp(\hat\gamma_{\rm dist}
    \cdot d)$ vs.\ haversine distance. Demand decays approximately
    50\% per km.}
  \label{fig:decay}
\end{figure}
