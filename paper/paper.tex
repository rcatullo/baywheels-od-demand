...
vertical range, so we expected this covariate to be marginal: Lyft
places stations on flat, accessible ground rather than atop hills,
limiting dock-to-dock elevation differences by design.
...
negligible.  Beyond the generally flat Bay Area topography, there is a
structural reason: Lyft deliberately locates docks on level,
accessible ground and avoids placing stations atop steep hills.  This
operator constraint suppresses dock-to-dock elevation differences
by design, so the signal cannot appear in the data regardless of how
strong a biological effect exists.  A similar model applied to a
system with hilly station siting might show a stronger effect.
